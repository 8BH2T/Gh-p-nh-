...
const Auth: React.FC<AuthProps> = ({ onLogin, onSignup }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (authView === 'forgotPassword') {
      ...
      setResetMessage('Nếu tài khoản của bạn tồn tại, một liên kết đặt lại mật khẩu đã được gửi đến email của bạn.');
      return;
    }
  ...
  const switchView = (view: 'login' | 'signup' | 'forgotPassword') => {
    setAuthView(view);
    setEmail('');
    setPassword('');
    setError('');
            ...
            {authView === 'login' && (
                <div className="flex items-center justify-end">
                    <div className="text-sm">
                        <button type="button" onClick={() => switchView('forgotPassword')} className="font-medium text-indigo-400 hover:text-indigo-300">
                            Quên mật khẩu?
                        </button>
                    </div>
                </div>
  ...
  const renderFooter = () => {
    if (authView === 'forgotPassword') {
      return (
        <button onClick={() => switchView('login')} className="text-sm text-indigo-400 hover:text-indigo-300">
    ...
    if (authView === 'login') {
      return (
        <button onClick={() => switchView('signup')} className="text-sm text-indigo-400 hover:text-indigo-300">
  ...
  const getTitle = () => {
    switch (authView) {
      ...
      case 'forgotPassword': return 'Đặt lại mật khẩu của bạn';
    }
  }
